...
func main() {
	...
	case "version":
		cliVersion()
	default:
		fmt.Fprintf(os.Stderr, "Usage: boxcutter-host <run|status|bootstrap|pull|upgrade|version>\n")
		os.Exit(1)
	}
}
...
//
//	boxcutter-host pull <node|orchestrator> [--tag TAG]
func cliPull() {
	cfg := defaultConfig()
	...
	// Decompress the zstd file to get the QCOW2
...
func upgradeNodes(cfg HostConfig, state *cluster.State, basePath string, meta *oci.ImageMetadata) {
	...
	// For each existing node, create a replacement with the new image
	for _, oldNode := range state.Nodes {
		fmt.Printf("Upgrading %s...\n", oldNode.ID)

		// Create new node with next available number
		...
		// Create COW disk from pulled base image
		...
		// Check if slim cloud-init ISO exists (user must generate it)
		...
		// Wait for new node to be healthy
...
func upgradeOrchestrator(cfg HostConfig, state *cluster.State, basePath string, meta *oci.ImageMetadata) {
	...
	// Create new disk from base image
...
func cliVersion() {
	...
	fmt.Println("\nLatest available:")
	for _, vmType := range []string{"node", "orchestrator"} {
		...
		if err != nil {
			fmt.Printf("  %s: (unable to check: %v)\n", vmType, err)
		} else {
			v := meta.Version
			if v == "" {
				v = meta.Digest[:12]
...
// parsePullArgs extracts vmType and tag from os.Args for pull/upgrade commands.
func parsePullArgs() (string, string) {
	if len(os.Args) < 3 {
		fmt.Fprintf(os.Stderr, "Usage: boxcutter-host %s <node|orchestrator|all> [--tag TAG]\n", os.Args[1])
		os.Exit(1)
	}

	vmType := os.Args[2]
	if vmType != "node" && vmType != "orchestrator" && vmType != "all" {
		fmt.Fprintf(os.Stderr, "VM type must be 'node', 'orchestrator', or 'all'\n")
		os.Exit(1)
	}
	// "all" is only valid for upgrade
	if vmType == "all" && os.Args[1] == "pull" {
		fmt.Fprintf(os.Stderr, "Cannot pull 'all' — specify 'node' or 'orchestrator'\n")
		os.Exit(1)
	}
...
// createCOWDisk creates a QCOW2 copy-on-write disk backed by the base image.
func createCOWDisk(basePath, diskPath, size string) error {
	cmd := fmt.Sprintf("qemu-img create -f qcow2 -b %s -F qcow2 %s %s",
		basePath, diskPath, size)
	return runShell(cmd)
}

// decompressZstd decompresses a .zst file to the given output path.
...
// waitForNodeHealth polls a node's health endpoint until it responds.
