...
import type { VM, TapegunActivity } from '@/lib/types'

export default function Dashboard() {
  const [vms, setVMs] = useState<VM[]>([])
  const [activity, setActivity] = useState<Record<string, TapegunActivity>>({})
    ...
    const poll = async () => {
      try {
        const [vmData, actData] = await Promise.all([
          ...
          fetch('/api/tapegun/activity').then(r => r.json()),
        ])
        setVMs(Array.isArray(vmData) ? vmData : [])
        const actMap: Record<string, TapegunActivity> = {}
        if (Array.isArray(actData)) {
          for (const a of actData) actMap[a.name] = a
        }
        setActivity(actMap)
      } catch {}
    }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        ...
        <StatCard label="Running" value={vms.filter(v => v.status === 'running').length} />
        <StatCard label="Nodes" value={[...new Set(vms.map(v => v.node_name).filter(Boolean))].length} />
      </div>
      ...
      <div className="bg-gray-900 rounded-lg border border-gray-800 overflow-hidden">
              ...
              return (
                <tr key={vm.name} className="border-b border-gray-800/50 hover:bg-gray-800/50">
                  <td className="px-4 py-3">
                    ...
                    {vm.description && (
                      <div className="text-xs text-gray-500 mt-0.5">{vm.description}</div>
                    )}
                  </td>
                  <td className="px-4 py-3">
        ...
        {vms.length === 0 && (
          <div className="text-center py-8 text-gray-500">No VMs found</div>
        )}
      </div>
    </div>
  )
